...
public class PremierLeagueDAO {
	...
	public List<Player> getVertici(int matchId, Map<Integer, Player> idMap){
		String sql = "SELECT * "
				+ "FROM actions a "
				+ "WHERE a.MatchID = ?";
	...
	public List<Adiacenza> getAdiacenze(int matchID, Map<Integer, Player> idMap){
		...
		try {
			...
			while (res.next()) {
				if(idMap.containsKey(res.getInt("p1")) && idMap.containsKey(res.getInt("p2"))) {
					Adiacenza adiacenza= new Adiacenza(idMap.get(res.getInt("p1")), idMap.get(res.getInt("p2")), res.getDouble("peso"));
